...
MONGO_PORT = 27017
MONGO_DB = 'agc2019'
STATIC_FOLDER = '/data' # Change this path to webserver/static
...
def save_article(article):
    ...
    if 'thumbnail' in article and article['thumbnail'] != '':
        if article['thumbnail'][:4] != 'http':
            ...
            shutil.copyfile(os.path.join("voa_crawler", article['thumbnail']), thumbnail_path)
    ...
    if 'audio' in article and article['audio'] != '':
        if article['audio'][:4] != 'http':
            ...
            shutil.copyfile(os.path.join("voa_crawler", article['audio']), audio_path)
